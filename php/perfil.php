    ...
    <main class="container" style="min-width: 60%">
      <section class="card-perfil">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <h2>Meu perfil</h2>
          <a href="../php/post_individual.php"
            style="padding: 8px 8px; background-color: transparent; color: black; border: 1px solid black; border-radius: 6px; text-decoration: none; font-size: 0.9rem;">
            <i class="fas fa-list" style="margin-right: 4px; font-size: 0.9rem;"></i> Minhas Postagens
          </a>
        </div>
        <?php if (!empty($usuario['foto']) && file_exists("uploads/" . $usuario['foto'])): ?>
          <img src="<?= "uploads/" . htmlspecialchars($usuario['foto']) ?>" alt="<?= htmlspecialchars($usuario['nome']) ?>"
            class="foto-perfil">
        <?php endif; ?>
        ...
        <?php if (!empty($curso)): ?>
          <p>
            <strong><?= $usuario['tipoUsuario'] == 'Aluno' || $usuario['tipoUsuario'] == 'Ex-aluno' ? 'Curso' : '' ?></strong>
            <?= htmlspecialchars($curso) ?></p>
        <?php endif; ?>

        <?php if (!empty($anoConclusao)): ?>
          <p>
            <strong><?= $usuario['tipoUsuario'] == 'Aluno' || $usuario['tipoUsuario'] == 'Ex-aluno' ? 'Ano de Conclusão' : '' ?></strong>
            <?= htmlspecialchars($anoConclusao) ?></p>
        <?php endif; ?>
